...
user_activity = {}
def add_user_activity_dict(id: int):
    if id not in user_activity.keys():
        user_activity[id] = {'sent':0, 'received':0}

def getCallbackQueryButtonText(query: types.CallbackQuery):
    for row in query.message.reply_markup.inline_keyboard:
        for reply_markup in row:
            if query.data == reply_markup.callback_data:
...
class LoggingMiddleware(BaseMiddleware): 
    ...
    async def on_pre_process_message(self, message: types.Message, data: dict):
        ...
        if message.text:
            self.logger.info(f"Received message [ID:{message.message_id} MSG:{message.text[:30]}] from [{message.from_user.full_name}:{message.chat.id}]")
        elif message.photo:
            self.logger.info(f"Received message [ID:{message.message_id} MSG:PHOTO] from [{message.from_user.full_name}:{message.chat.id}]")
        else:
            self.logger.info(f"Received message [ID:{message.message_id} MSG:{message.content_type}] from [{message.from_user.full_name}:{message.chat.id}]")

        add_user_activity_dict(message.chat.id)
        user_activity[message.chat.id]['sent'] += 1
    ...
    async def on_pre_process_inline_query(self, inline_query: types.InlineQuery, data: dict):
        self.logger.info(f"Received inline query [ID:{inline_query.id}] "
                         f"from user [ID:{inline_query.from_user.id} NAME:{inline_query.from_user.full_name}]")
    ...
    async def on_pre_process_chosen_inline_result(self, chosen_inline_result: types.ChosenInlineResult, data: dict):
        self.logger.info(f"Received chosen inline result [Inline msg ID:{chosen_inline_result.inline_message_id}] "
                         f"from user [ID:{chosen_inline_result.from_user.id} NAME:{chosen_inline_result.from_user.full_name}] "
    ...
    async def on_pre_process_callback_query(self, callback_query: types.CallbackQuery, data: dict):
        if callback_query.message:
            text = (f"Received callback query [BTN:{getCallbackQueryButtonText(callback_query)}] "
                    f"from user [ID:{callback_query.from_user.id}  NAME:{callback_query.from_user.full_name}] ")
            ...
            add_user_activity_dict(callback_query.message.chat.id)
            user_activity[callback_query.message.chat.id]['sent'] += 1
        else:
    ...
    async def on_pre_process_shipping_query(self, shipping_query: types.ShippingQuery, data: dict):
        self.logger.info(f"Received shipping query [ID:{shipping_query.id}] "
                         f"from user [ID:{shipping_query.from_user.id} NAME:{shipping_query.from_user.full_name}]")
    ...
    async def on_pre_process_pre_checkout_query(self, pre_checkout_query: types.PreCheckoutQuery, data: dict):
        self.logger.info(f"Received pre-checkout query [ID:{pre_checkout_query.id}] "
                         f"from user [ID:{pre_checkout_query.from_user.id} NAME:{pre_checkout_query.from_user.full_name}]")
    ...
    async def on_pre_process_poll_answer(self, poll_answer, data):
        self.logger.info(f"Received poll answer [ID:{poll_answer.poll_id}] "
                         f"from user [ID:{poll_answer.user.id} NAME:{poll_answer.from_user.full_name}]")
